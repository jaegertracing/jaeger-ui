...
>
> **The kind mapping here is lossy in one direction.** A span sent with `kind: 0`
> (`SPAN_KIND_UNSPECIFIED`) comes back from this endpoint as `"internal"`, not
> `"unspecified"` — so `kind: 0` and `kind: 1` (`SPAN_KIND_INTERNAL`) are indistinguishable
> on `/api/v3/operations`. A shared `SpanKind` mapper therefore needs a number→string table
> and a string→number table that are **not** inverses of each other, and it cannot treat this
> endpoint as authoritative for a span's kind.
...
Do not expect the protojson enum *name* (`"SPAN_KIND_SERVER"`) on this endpoint, as the sketch's comment above assumed. That form would come from gogo protojson, which is not what serialises this response. Note the contrast with `/api/v3/operations` (see Milestone 3.1), which is a Jaeger-defined message rather than OTLP and returns `spanKind` as a lower-case string — the two endpoints genuinely differ, so a shared `SpanKind` mapper needs to handle both, and must account for `/api/v3/operations` collapsing `UNSPECIFIED` onto `"internal"`.
...
Note the token is `bytes`, plural — **not** the registered OpenAPI `byte` format that means base64. `protoc-gen-openapi` emits the protobuf type name verbatim, so `format: bytes` is not a recognised OpenAPI format at all. That distinction matters for the fix below: correcting the spec means replacing a non-standard token, not swapping one standard format for another. It also explains why the generated client happens to work today — `openapi-zod-client` does not recognise `bytes`, so it degrades to a plain `z.string()` and never attempts base64 decoding.
...
- [ ] The `GET /api/v3/traces/{traceId}` `200` response contradicts its own description. The response schema `$ref`s `opentelemetry.proto.trace.v1.TracesData` **unwrapped**, while the same operation's `description` states *"the JSON response over HTTP is wrapped into result envelope `{"result": ...}`"*. The wrapped form is what the server actually sends (see Milestone 3.2), so the schema is the wrong half. A client generated from this spec expects `resourceSpans` at the top level and will not find it. Fixing this in jaeger-idl would also let the generated client type the envelope instead of the UI hand-rolling it.
