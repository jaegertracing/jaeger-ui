...
The **target architecture** (layers, data flow, and where each kind of state should live) is documented in **[RFC 0006: Target State Management Architecture](./0006-target-state-management-architecture.md)**; how the code is wired while this migration runs is in **[ADR-0005](../adr/0005-current-state-management-architecture.md)**. **This section** is the **migration checklist**: what to deliver, in what order, and how to verify or roll back.
...
**New Zustand store** — delivered as *two* stores rather than the single one sketched below, split by lifetime and both re-exported from `TraceTimelineViewer/store.ts`: `useLayoutPrefsStore` (`store.layout.ts`) holds the persisted layout fields, `useTraceTimelineStore` (`store.timeline.ts`) holds the per-trace interaction fields, so a trace change can reset the latter without touching the former. `prunedServices` was added to the timeline store later by [ADR-0009](../adr/0009-service-filter-trace-timeline.md). The single-store shape as originally proposed:
...
**Goal**: Replace "dispatch → promise middleware → reducer → selector" with query hooks and a cache key. Coordinate with [RFC 0002](./0002-otel-native-jaeger-ui.md) / `api/v3` where applicable.
...
**As delivered** (`src/hooks/useTraceLoading.ts`): `useTrace(traceId)` for a single trace and `useTraces(ids)` for the compare cohort, both on the `['trace', id]` key with `staleTime: Infinity`. The fetch still goes through the **legacy** route — `JaegerAPI.fetchTrace` (`/api/traces/:id`) piped through `transformTraceData(...).asOtelTrace()`. Only the state layer moved off Redux here; switching the transport to `/api/v3/traces/{trace_id}` and a native OTLP parser is [RFC 0002](./0002-otel-native-jaeger-ui.md) Milestone 3.2.
...
The residual surface is now small enough to enumerate:
...
- Supersede [ADR-0005](../adr/0005-current-state-management-architecture.md) once Redux is no longer part of the runtime.
