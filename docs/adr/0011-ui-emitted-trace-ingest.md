...
| ✅ **Same-origin ingest on the UI server** ← **chosen** | OTLP/HTTP proxy in `jaeger-query` at `/api/otlp/v1/*`. | No CORS. One deployment shape (query port already exposed). Auth piggybacks on whatever protects `/api/traces`. Composes with the OTLP-native migration in [RFC 0002](../rfc/0002-otel-native-jaeger-ui.md). |
