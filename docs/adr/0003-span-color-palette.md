...
- **Known gap — the values are correct on load, but lag a theme toggle.** The resolved RGB is baked into a canvas draw or an inline style, so it is only as current as the last render, and the DOM read has to happen after `data-theme` has been updated. Neither holds on a toggle:
  - `TraceFlamegraph` (search-dimmed frames) and `TraceGraph/OpNode` (service-mode backgrounds) do not subscribe to the theme at all, so they keep the previous colors until something else re-renders them.
  - `CanvasSpanGraph` does subscribe, via `useThemeMode`, and still ends up one toggle behind: it redraws in a `useEffect` keyed on the mode, but `ThemeProvider` writes `document.body.dataset.theme` in a `useEffect` of its own, and React runs a parent's effects *after* its descendants'. The canvas therefore reads the palette before the attribute that selects it has changed. Verified in the browser — after toggling to light, the canvas still contained `#da1e28` (dark-6); after toggling to dark, it still contained `#fa4d56` (light-6).

  Reloading the page always produces the correct palette, because then no toggle is involved. Fixing the toggle case means making the theme attribute land before descendant effects run, which is a change to `ThemeProvider` rather than to the palette.
