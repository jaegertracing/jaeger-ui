...
- [ADR-0005: Current State Management Architecture](./0005-current-state-management-architecture.md) - Documentation of the existing multi-layered state management (TanStack Query, Zustand, URL, Local Storage, and the Redux residue); to be retired when Redux is gone
...
- [ADR-0010: Layout Settings Priority Stack](./0010-layout-settings-priority-stack.md) - Cascading priority system (URL > heuristics > localStorage) for resolving layout settings without corrupting user preferences; proposed, not yet implemented
