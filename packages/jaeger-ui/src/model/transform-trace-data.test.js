...
// The rest of the app walks the parent and child pointers this transform hands out with
// no visited set of its own: SpanTreeOffset climbs parentSpan to build indent guides, the
// flamegraph and DDG descend childSpans. A cycle left in either direction hangs them, so
// the shape itself is asserted rather than just the flat span list.
function expectAcyclicSpanGraph(trace) {
  const otel = trace.asOtelTrace();
  const limit = otel.spans.length;
  ...
  const descend = (span, depth) => {
    expect(depth).toBeLessThanOrEqual(limit);
    span.childSpans.forEach(child => descend(child, depth + 1));
  };
  otel.rootSpans.forEach(root => descend(root, 0));
}
...
describe('transformTraceData()', () => {
  ...
  it('should place every span of a three-span cycle exactly once', () => {
    ...
    // The promoted span is no longer anyone's child: 'a' was 'c's parent, and the edge
    // that closed the cycle is gone in both directions.
    expect(result.spanMap.get('a').childSpans).toEqual([]);
    expect(result.spanMap.get('a').hasChildren).toBe(false);
    const otelC = result.asOtelTrace().spanMap.get('c');
    expect(otelC.parentSpanID).toBeUndefined();
    expect(otelC.parentSpan).toBeUndefined();
    // The reference itself survives for display, as a link.
    expect(otelC.links.map(l => l.spanID)).toEqual(['a']);
  });
  ...
  it('should promote one root per disjoint cycle and leave a normal subtree untouched', () => {
    ...
    // Both roots are traversed in startTime order, the cycle's promoted member last.
  ...
  it('should promote a span on the cycle and keep a dangling descendant attached', () => {
    const traceID = 'ff11002233445566778899aabbccddee';
    const mk = (spanID, parentID, offset) => ({
      traceID,
      spanID,
      operationName: `op-${spanID}`,
    ...
    // x and y form a cycle; tail hangs below y and is the earliest span of the three.
    // The tail is rootless too, but its parent edge is a legitimate one, so the climb
    // from it has to walk past the tail and land on a member of the cycle itself.
    const result = transformTraceData({
      traceID,
      processes,
      spans: [mk('x', 'y', 30), mk('y', 'x', 20), mk('tail', 'y', 0)],
    });

    expect(result.spans.length).toBe(3);
    expect(result.rootSpans.map(s => s.spanID)).toEqual(['y']);
    expect(result.spans.map(s => s.spanID)).toEqual(['y', 'tail', 'x']);
    expect(result.spans.map(s => s.depth)).toEqual([0, 1, 1]);
    expect(result.spanMap.get('y').warnings).toContain(CYCLIC_PARENT_WARNING);
    // The tail keeps the parent it always had; only the cycle edge was cut.
    expect(result.spanMap.get('tail').warnings).not.toContain(CYCLIC_PARENT_WARNING);
    expect(result.asOtelTrace().spanMap.get('tail').parentSpan.spanID).toBe('y');
    expectAcyclicSpanGraph(result);
  });

  it('should render a span that references itself', () => {
    const traceID = '99aabbccddeeff00112233445566778';
    // A one-span cycle: the span is its own parent, so it is its own child and nothing
    // reaches it. The cut has nowhere to go but back to the same span.
    const selfRef = {
      traceID,
      spanID: 'self',
      operationName: 'self-referencing',
      references: [{ refType: 'CHILD_OF', traceID, spanID: 'self' }],
      startTime,
      duration,
      tags: [],
      logs: [],
    ...
    const result = transformTraceData({ traceID, processes, spans: [selfRef] });

    expect(result.spans.map(s => s.spanID)).toEqual(['self']);
    expect(result.rootSpans.map(s => s.spanID)).toEqual(['self']);
    expect(result.spans[0].hasChildren).toBe(false);
    expect(result.spans[0].childSpans).toEqual([]);
    expect(result.spans[0].warnings).toContain(CYCLIC_PARENT_WARNING);
    expect(result.duration).toBe(duration);
    expectAcyclicSpanGraph(result);
  });
