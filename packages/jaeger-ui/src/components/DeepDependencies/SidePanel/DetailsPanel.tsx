...
export function UnconnectedDetailsPanel(props: TProps) {
  ...
  const [columnDefs, setColumnDefs] = useState<TColumnDefs | undefined>();
  const [details, setDetails] = useState<TDetails | undefined>();
  ...
  useEffect(() => {
    ...
    setDetailsLoading(false);
    fetchDetails();
  }, [operation, service, decorationSchema, fetchDetails]);
