...
export const UnconnectedTraceDiffGraph: React.FC<Props> = React.memo(props => {
  const { a, b, uiFind = '' } = props;

  const layoutManager = useMemo(() => new LayoutManager({ useDotEdges: true, splines: 'polyline' }), []);
  const cacheAsFn = useMemo(() => cacheAs.makeScope(), []);

  useEffect(() => {
    // componentWillUnmount lifecycle equivalent
    return () => {
      ...
      <Digraph
        ...
        layers={[
          ...
          },
          {
            ...
            defs: [{ localId: 'arrow' }],
            markerEndId: 'arrow',
            setOnContainer: cacheAsFn('edges/container', [
              scaleOpacity,
              scaleStrokeOpacity,
              { stroke: '#444' },
            ]),
