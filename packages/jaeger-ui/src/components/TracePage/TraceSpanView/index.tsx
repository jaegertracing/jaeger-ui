...
const TraceSpanView: React.FC<Props> = props => {
  ...
  const [filteredData, setFilteredData] = useState<ReadonlyArray<IOtelSpan>>(trace.spans);
  ...
  const uniqueOperationNameOptions = useCallback(() => {
    let operationNamesListLocal: string[];
    if (filters.serviceName) {
      operationNamesListLocal = filters.serviceName.flatMap(svc => serviceToOperationsMap.get(svc) || []);
    } else {
      operationNamesListLocal = operationNamesList;
    }
    return [...new Set(operationNamesListLocal)];
  }, [filters.serviceName, serviceToOperationsMap, operationNamesList]);

  const onFilteredChangeCustom = useCallback(
    (selectedValues: string[], filterType: FilterType) => {
      const newFilters = { ...filters, [filterType]: selectedValues };

      const newFilteredData = trace.spans.filter(span => {
  ...
  return (
    <div>
      ...
      >
        ...
        <Form.Item className="reset-filter" style={{ flex: '0 0 auto' }}>
...
export default React.memo(TraceSpanView);
