...
export default function TraceSpanView(props: Props) {
  const { trace, useOtelTerms } = props;

  // Compute initial data from trace (memoized for performance)
  const { serviceNamesList, operationNamesList, serviceToOperationsMap, maxDuration } = useMemo(() => {
    ...
    const svcToOpsMap = new Map<string, Set<string>>();
    ...
    trace.spans.forEach(span => {
      ...
      if (!svcToOpsMap.has(serviceName)) {
        svcToOpsMap.set(serviceName, new Set<string>());
      }
      svcToOpsMap.get(serviceName)!.add(span.name);
    });

    // Sort alphabetically for better UX
    const sortedServiceNamesList = [...serviceNamesSet].sort();
    const sortedOperationNamesList = [...operationNamesSet].sort();

    // Convert operation sets to sorted arrays
    const sortedServiceToOperationsMap = new Map<string, string[]>();
    svcToOpsMap.forEach((operations, serviceName) => {
      sortedServiceToOperationsMap.set(serviceName, [...operations].sort());
    });

    // Compute max duration once for the entire trace
    const computedMaxDuration = Math.max(...trace.spans.map(s => s.duration), 1);

    return {
      serviceNamesList: sortedServiceNamesList,
      operationNamesList: sortedOperationNamesList,
      serviceToOperationsMap: sortedServiceToOperationsMap,
      maxDuration: computedMaxDuration,
    };
  }, [trace.spans]);
  ...
  const uniqueOperationNameOptions = useMemo(() => {
  ...
  }, [filters.serviceName, serviceToOperationsMap, operationNamesList]);

  const onFilteredChangeCustom = useCallback((selectedValues: string[], filterType: FilterType) => {
    setFilters(prevFilters => ({ ...prevFilters, [filterType]: selectedValues }));
  }, []);

  const columns: ColumnProps<IOtelSpan>[] = useMemo(
    () => [
      {
        title: 'Service Name',
        width: '25%',
